...
.SYNOPSIS
    ...
    Authentication via Enterprise App + Client Secret (Client Credentials Flow).
    Fully local executable version.
.AUTHOR
    Maurice Flöthmann
.COPYRIGHT
...
# MAIN EXECUTION
# ============================================================
Write-Log "=== M365 Service Health Monitor started (Local) ==="

# ==================== Step 1: Validate Configuration ====================
Write-Log "--- Step 1: Configuration ---"

if ([string]::IsNullOrWhiteSpace($Config.TenantId) -or 
    [string]::IsNullOrWhiteSpace($Config.ClientId) -or 
    [string]::IsNullOrWhiteSpace($Config.ClientSecret)) {
    Write-Log "ERROR: Please fill in TenantId, ClientId and ClientSecret in the Config section!" -Lvl 'ERROR'
    throw "Configuration is incomplete"
}

Write-Log "TenantId  : $($Config.TenantId)"
Write-Log "ClientId  : $($Config.ClientId)"
Write-Log "Recipient : $($Config.RecipientEmail)"
Write-Log "Sender    : $($Config.SenderEmail)"

# ==================== Step 2: Authentication (Client Credentials) ====================
...
try {
    ...
    Write-Log "Successfully authenticated with Enterprise App (Client Secret)"
}
catch {
    Write-Log "Authentication failed: $($_.Exception.Message)" -Lvl 'ERROR'
    throw
}
...
# ==================== Step 5: Retrieve Message Center ====================
Write-Log "--- Step 5: Retrieve Message Center ---"

$filterDate = (Get-Date).AddDays(-$Config.MessageCenterDaysBack).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")
...
# Message Center
if ($messages) {
    $html += "<h2>Recent Message Center Announcements (last $($Config.MessageCenterDaysBack) days)</h2>"
...
try {
    ...
    Write-Log "Email sent successfully to $($Config.RecipientEmail)"
}
catch {
